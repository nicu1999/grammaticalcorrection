...
_URLS = {
    "token_tagging": ['./wiki_dirty_gpt_100000_shuffled_inter_128_add.txt', './golden_corpus_add_verify.txt']
}
    ...
    def _info(self):
        if self.config.name == "token_tagging":
            features = datasets.Features({"input_ids": datasets.features.Sequence(datasets.Value('int32')), 
                                          ...
                                          'labels': datasets.features.Sequence(feature=datasets.features.ClassLabel(num_classes=2, names=['ok', 'add'], id=None), length=-1, id=None),
                                        })
