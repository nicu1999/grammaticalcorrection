...
std::vector<int> removeZeroAfterOne(const std::vector<int>& inputVec) {
    std::vector<int> outputVec;
    bool removeNextZero = false;
...
std::vector<int> traverse_matrix(std::vector<std::vector<Cell>> d, int lenS, int lenT, const std::vector<int> source, const std::vector<int> target) {
    ...
    std::cout << add_mask.size() <<endl;

    add_mask = removeZeroAfterOne(add_mask);
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream outfile("corpus/results_2/golden_corpus_128_filtered_well_formed_no_duplicates_add.txt");
